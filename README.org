...
To prevent that problem we've implemented passing of last steps model output as well as a notes section persisting over steps. See the customisation section about disabling it, but given that this is a problem we're introducing by having the model re-interpret the picture and what has been done for every step it probably is fair to provide that. Disabling it provides a more artsy look, though.
